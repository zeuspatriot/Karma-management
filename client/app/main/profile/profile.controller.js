...
angular.module('karmaApp')
  .controller('ProfileCtrl', function ($scope, Auth, User, Log, Rules, $routeParams) {
	...
	var getUser = function (){
		...
		$scope.users = User.query(function(){a()});
	};
	getUser();
  ...
  $scope.newRuleTogle = function(){
	...
	var a = function(){
		...
		$scope.users.forEach(function(el, index){
			if(el.name == $scope.user.name){
				console.log($scope.users[index+1]);
				$scope.users[index+1] ? $scope.sortedUsers.push($scope.users[index+1]) : $scope.sortedUsers.push($scope.user);
				if($scope.users[index-1]){
					$scope.sortedUsers[0].name != $scope.user.name ? $scope.sortedUsers.push($scope.user, $scope.users[index-1]) : $scope.sortedUsers.push($scope.users[index-1]);
